...
export const useCart = () => {
    ...
    const initialCart = () : CartItem[] => { // Iniciamos el carrito
      ...
      function addToCart(item : Guitar) { // funcion para agregar items al carrito de compras
        ...
        if(itemExists >= 0) { // Si existe en el carrito
          ...
          const newItem : CartItem = {...item, quantity : 1} // Creamos una nueva variable y especificarle que va a ser de tipo CartItem y de esa forma va a tener una copia de item y le agregamos el nuevo atributo queantity
          setCart([...cart, newItem]) // Seteamos el nuevo elemento en el carrito
        }
      }
    
      function removeFromCart(id : Guitar['id']) { // Funcion para eliminar un item, en el parametro le aclaramos que solo queremos el ID del type Guitar
        setCart(prevCart => prevCart.filter(guitar => guitar.id !== id))
      }
    
      function increaseQuantity(id : Guitar['id']) { // Funcion para incrementar la cantidad sobre un item en el carrito
      ...
      function decreaseQuantity(id : Guitar['id']) { // Funcion para decrementar la cantidad sobre un item en el carrito
        const updatedCart = cart.map( item => { // el map retorna un nuevo Arreglo
